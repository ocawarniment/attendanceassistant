...
function scanTable() {
	storage.get(null, function(result) {
        // get current homeroomMetrics
        var currentHomeroomArray = result.homeroomArray;
		...
		var studentCountString; // string to cut down to extract the number
		var studentcount; // integer to use in loops
		studentCountString = document.getElementsByClassName("sas-student-count ng-binding")[0].children[0].innerHTML;
		studentCountString = studentCountString.match(/\d*/g)[0];
		studentCount = studentCountString;
		...
		if (overdueFound == true) {
			...
			while (i <= studentCount - 1) {
				...
				var studentID;
				studentID = studentRows[i].getElementsByTagName("td")[0].innerText;
				...
				currentHomeroomArray['ST' + studentID]['oldOverdue'] = currentHomeroomArray['ST' + studentID]['overdue'];
				currentHomeroomArray['ST' + studentID]['overdue'] = studentOverdue;
				
				i++;
			}
					
			// set the array to access it on the popup
			storage.set({'homeroomArray': currentHomeroomArray});
