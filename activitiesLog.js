...
function checkAutomation(){
	chrome.storage.local.get(null, (result) => {
		if((url.includes("&ccp=") || url.includes("&cte=")) && result.schoolVars.name == 'oca') {
			...
			// get the time
			if(time == 'auto') {
				chrome.runtime.sendMessage({type: 'scrapeValue', url: `https://www.connexus.com/dataview/${result.schoolVars.truancy.dataViewID}?idWebuser=` + studentID, cssSelector: `#${elemId}`}, async (response)=>{
	...
	try {
		...
		if((type == "cte" || type == "ccp") && !document.querySelector('#btnUnapprove')){
			...
			cteccpAdjustBtn.onclick = function(){
				...
				if(submit) {chrome.runtime.sendMessage({type: 'openPage', url: adjustUrl, focused: false, closeSender: true});}
			};
